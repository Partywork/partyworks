...
import { SingleEventSource } from "../lib/EventSource";
...
export default class ReconnectingWebSocket extends (EventTarget as TypedEventTarget<WebSocketEventMap>) {
  ...
  private status:
    ...
    | "Disconnected"
    | "failed" = "Initial";
  public eventSource = new SingleEventSource<
    | "Initial"
    | "Connecting"
    | "Connected"
    | "Reconnecting"
  ...
  private _connect() {
    ...
    this.status = this.status === "Initial" ? "Connecting" : "Reconnecting";
    this.eventSource.notify(this.status);
